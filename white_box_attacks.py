...
class ImperceptibleWhiteBoxAttacks:
    def __init__(
        self,
        model,
        ...
        self.global_optimal_delta.to(self.device)
    ...
    def _check_params(self) -> None:
        ...
        if not isinstance(self.global_max_length, int):
            raise ValueError(
                "The length of the longest audio signal must be of type int."
            )
        if self.global_max_length <= 0:
            raise ValueError(
                "The length of the longest audio signal must be greater than 0."
            )
        ...
        if self.initial_rescale <= 0.0:
            raise ValueError(
                "The initial rescale coefficient must be greater than 0.0."
            )
        ...
        if not isinstance(self.hop_length, int):
            raise ValueError(
                "Number of audio samples between adjacent STFT columns must be of type int."
            )
        if self.hop_length <= 0:
            raise ValueError(
                "Number of audio samples between adjacent STFT columns must be greater than 0."
            )
        ...
        if self.win_length > self.n_fft:
            raise ValueError(
                "Length of the window must be smaller than or equal to FFT window size."
            )
    ...
    def generate_adv_sample(
        ...
        for m in range(num_batch):
            ...
            for i in range(len(adv_x_batch)):
                adv_x[batch_index_1 + i] = adv_x_batch[
                    i, : len(adv_x[batch_index_1 + i])
                ]
    ...
    def _generate_batch(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        successful_adv_input_1st_stage, original_input = self._attack_1st_stage(
            x=x, y=y
        )
        successful_perturbation_1st_stage = (
            successful_adv_input_1st_stage
            - torch.tensor(original_input).to(self.model.device)
        )
    ...
    def _forward_1st_stage(
        ...
        "torch.Tensor", "torch.Tensor", np.ndarray, "torch.Tensor", "torch.Tensor"
    ]:
        """
        ...
        local_delta_rescale *= torch.tensor(rescale).to(self.model.device)
        ...
        # loss, decoded_output = self.model.compute_loss_and_decoded_output(
        #     masked_adv_input=masked_adv_input,
        #     original_output=original_output,
        #     real_lengths=real_lengths,
        # )

        # pass perturbed input to model
        masked_adv_input = masked_adv_input.float()
        original_output = (
            torch.tensor(original_output).to(self.model.device).unsqueeze(1)
        )
        original_output = original_output.float()
        loss = self.criterion(model(masked_adv_input), original_output)
        decoded_output = torch.sigmoid(model(masked_adv_input)) + 0.5
        decoded_output = decoded_output.int()
    ...
    def _attack_2nd_stage(
        ...
        for iter_2nd_stage_idx in range(self.max_iter_2):
            ...
            # Total loss
            loss = (
                loss_1st_stage.type(torch.float64)
                + torch.tensor(alpha).to(self.model.device) * loss_2nd_stage
            )
            ...
            for local_batch_size_idx in range(local_batch_size):
                if decoded_output[local_batch_size_idx] == y[local_batch_size_idx]:
                    if loss_2nd_stage[local_batch_size_idx] < torch.tensor(
    ...
    def _forward_2nd_stage(
        ...
        for i, _ in enumerate(theta_batch):
            ...
            loss = torch.mean(
                relu(
                    psd_transform_delta
                    - torch.tensor(theta_batch[i]).to(self.model.device)
                )
            )
            losses.append(loss)
    ...
    def _psd_transform(
        self, delta: "torch.Tensor", original_max_psd: np.ndarray
    ) -> "torch.Tensor":
        """
        ...
        psd = (
            torch.pow(
                torch.tensor(10.0).type(torch.float64),
                torch.tensor(9.6).type(torch.float64),
            ).to(self.model.device)
            / torch.reshape(
                torch.tensor(original_max_psd).to(self.model.device), [-1, 1, 1]
            )
...
if __name__ == "__main__":
    DATASET_PATH = "/mnt/storage/hanhnlh/ccs/dataset/WaveFake"     
    ...
    # Load val dataset
    val_ds = get_dataset(
        datasets_paths=[None, DATASET_PATH],
    )

    print(f"Loaded {len(val_ds)} audio files")
    ...
    # # Load Imperceptible Attacks
    # attack = ImperceptibleWhiteBoxAttacks(
    #     model=model,
    #     device=device,
    #     max_iter_1=1000,
    ...
    for i, sample in enumerate(val_loader):
        ...
        if y.item() == 1:
            ...
            pred = (pred + 0.5).int()  # 0 if spoof, 1 if bonafide
            x_adv = attack.generate_adv_sample(
                x.cpu().numpy(), torch.logical_not(y).cpu().numpy()
            )
            y_adv = model(torch.tensor(x_adv).to(device))
            y_adv = torch.sigmoid(y_adv)
            y_adv = (y_adv + 0.5).int()

            print(f"Original: {pred}, Adversarial: {y_adv}")

            # Save adversarial sample
            x_adv = x_adv[0]

            sample_path = "/".join(filepath[0].split("/")[:-1])
            sample_filename = filepath[0].split("/")[-1]
            # create folder if not exist
            if not os.path.exists(f"outputs_adv/{PREFIX}/"):
                os.makedirs(f"outputs_adv/{PREFIX}/")
            adv_sample_filename = (
                f"outputs_adv/{PREFIX}/" + os.path.splitext(sample_filename)[0] + ".npy"
            )
            # x_adv = torch.tensor(x_adv).unsqueeze(0).float().cpu()  # (1, WAVE_FAKE_SR)
            np.save(adv_sample_filename, x_adv)
            
            x_stats = [x.min().item(), x.max().item(), x.mean().item(), x.std().item()]
            x_adv_stats = [x_adv.min(), x_adv.max(), x_adv.mean(), x_adv.std()]
            row = {"x": filepath[0], "x_adv": adv_sample_filename, "y": y.item(), "y_adv": y_adv.item(), "x_stats": x_stats, "x_adv_stats": x_adv_stats }
            print(row)
            end_time = time.time()
            print(f"Time in seconds: {end_time - start_time}")
            # import pdb; pdb.set_trace()
            output_df.append(row)
            pd.DataFrame(output_df).to_csv(f"outputs_adv/{PREFIX}/adv_samples.csv", index=False)
